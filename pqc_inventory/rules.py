...
PYTHON_RULES: list[Rule] = [
    ...
    # Canonical names and legacy aliases are separate rules so same-site
    # alias pairs can be merged without double-counting inventory hits.
    Rule(
        id="py-ml-kem",
        family="ML-KEM",
        pattern=r"(?i)\bML[-_]?KEM(?:[-_]?\d+)?\b",
        risk="safe",
        languages=("python",),
        description="NIST ML-KEM post-quantum KEM",
    ),
    Rule(
        id="py-kyber",
        family="ML-KEM",
        pattern=r"(?i)\bKyber(?:512|768|1024)?\b",
        risk="safe",
        languages=("python",),
    ...
    Rule(
        id="py-ml-dsa",
        family="ML-DSA",
        pattern=r"(?i)\bML[-_]?DSA(?:[-_]?\d+)?\b",
        risk="safe",
        languages=("python",),
        description="NIST ML-DSA post-quantum signature",
    ),
    Rule(
        id="py-dilithium",
        family="ML-DSA",
        pattern=r"(?i)\bDilithium(?:2|3|5)?\b",
        risk="safe",
        languages=("python",),
        description="Dilithium legacy alias for NIST ML-DSA",
    ),
    Rule(
        id="py-slh-dsa",
        family="SLH-DSA",
        pattern=r"(?i)\bSLH[-_]?DSA(?:[-_]?\w+)?\b",
        risk="safe",
        languages=("python",),
        description="NIST SLH-DSA post-quantum signature",
    ),
    Rule(
        id="py-sphincs",
        family="SLH-DSA",
        pattern=r"(?i)\bSPHINCS\+?\b",
        risk="safe",
        languages=("python",),
        description="SPHINCS+ legacy alias for NIST SLH-DSA",
    ),
    Rule(
        id="py-falcon",
        family="PQC",
